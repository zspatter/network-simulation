...
def test_get_prioritized_patients():
    wait_list = wL.WaitList()
    patient1 = p.Patient('name1', 'illness1', p.Patient.PANCREAS, p.Patient.O_NEG, 500, 1,
                         wait_list)
    patient2 = p.Patient('name2', 'illness2', p.Patient.PANCREAS, p.Patient.AB_POS, 200, 1,
                         wait_list)
    patient3 = p.Patient('name3', 'illness3', p.Patient.PANCREAS, p.Patient.AB_POS, 300, 1,
                         wait_list)
    patient4 = p.Patient('name4', 'illness4', p.Patient.PANCREAS, p.Patient.AB_POS, 400, 1,
                         wait_list)
...
def test_remove_patient():
    ...
    assert len(wait_list.wait_list) is 0
    patient2 = p.Patient('name1', 'illness1', p.Patient.PANCREAS, p.Patient.O_NEG, 500, 1,
                         wait_list)
    wait_list.remove_patient(patient)
    assert len(wait_list.wait_list) is 1
