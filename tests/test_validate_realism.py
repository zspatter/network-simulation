...
_MODEL_STUB = {
    ...
    'non_use_rate': 0.21,
    'retransplant_share': 0.06,
    'waitlist_final': 100_000.0,
}


def test_comparison_rows_cover_every_target_metric():
    rows = validate_realism.comparison_rows(dict(_MODEL_STUB))

    assert len(rows) == 7
    labels = ' '.join(row.metric.lower() for row in rows)
    for token in ('transplant', 'living', 'death', 'removal', 'non-use', 're-transplant',
                  'wait-list'):
        assert token in labels
    # ratio is model / target
    non_use = next(r for r in rows if 'non-use' in r.metric.lower())
...
def test_format_table_renders_rates_and_counts():
    ...
    assert '%' in text  # the rate rows render as percentages
...
def test_run_validation_returns_full_scale_metrics():
    ...
    assert set(model) == {'deceased_transplants_per_year', 'living_transplants_per_year',
                          'deaths_per_year', 'removals_per_year', 'non_use_rate',
                          'retransplant_share', 'waitlist_final'}
    assert model['deceased_transplants_per_year'] > 0
    assert 0.0 <= model['non_use_rate'] <= 1.0
    assert 0.0 <= model['retransplant_share'] <= 1.0
